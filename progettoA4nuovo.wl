...
S:= Function[val, sboxGift[[val + 1]]];
StratoS[x_List] :=Map[S,x];
  (*unisciNibble[
    Map[
...
permutaNibble[x_, permutazione_] :=
  ...
  (*togli unisci e dividi in modo che dipenda dalla lunghezza della lista*)
 
	permutaNibble[nibble_List, permutazione_] := 	Flatten@Map[P[#,permutazione]&,Partition[nibble,Length[permutazione]/4]];
 P[x_,permutazione_]:=
 Map[FromDigits[#,2]&,Partition[
 Part[
    Flatten[IntegerDigits[x,2,4]],
    permutazione + 1
  ],4]]
...
differenzeDopoRound[alpha_Integer, nBits_Integer, rounds_Integer] := (*calcola le differenze di output raggiungibili partendo da alpha , usando nBits bit e rounds round.*)
  Module[
    {size, f},
(*size conterr\[AGrave] il numero totale degli input;
f conterr\[AGrave] la funzione ottenuta dopo aver applicato i round.*)
    size = 2^nBits;
(*
Nest: applica la stessa funzione pi\[UGrave] volte di seguito.
Nest[
    funzione,
    valoreIniziale,
    numeroDiVolte
...
*)
    f = Nest[
      Function[{g},
        Function[{x}, superboxRidotta[nBits][g[x]]]
      ],
      Function[{x}, x], (*funzione identit\[AGrave]. Cio\[EGrave] una funzione che restituisce esattamente quello che riceve. Es. Function[{x}, x][5] restituisce 5*)
      rounds
    ];
(* Parte dalla funzione identit\[AGrave] e aggiunge
   un round di superboxRidotta ad ogni iterazione,
   ottenendo la funzione corrispondente a "rounds" round. *)
    DeleteDuplicates[
      Map[
        Function[{x},
          xor[
  ...
  (*Per ogni x ottieni una beta, alla fine DeleteDuplicates elimina le beta ripetute.
Quindi il risultato finale \[EGrave] la lista delle beta diverse raggiungibili da alpha dopo rounds round*)

  differenzialeImpossibileQ[alpha_Integer, beta_Integer, nBits_Integer, rounds_Integer] :=
  Not[
    MemberQ[
      differenzeDopoRound[alpha, nBits, rounds],
      beta
    ](*chiede se beta \[EGrave] presente nell'insieme di tutte le beta raggiungibili*)
  ];(*restituisce True se \[EGrave] impossibile*)
  
  trovaDifferenzialeImpossibile[nBits_Integer, rounds_Integer] :=
  Module[
    {differenze, alpha},

    differenze = Range[1, 2^nBits - 1];
(*FirstCase significa trova il primo elemento di una lista che soddisfa una certa condizione.*)
(*FirstCase[
    lista,
    condizione,
    valoreSeNonTrova
    ...
    alpha =
      FirstCase[ 
        differenze,
        a_ /; Length[differenzeDopoRound[a, nBits, rounds]] < Length[differenze],
        Missing["NotFound"] (*se non trova nulla restituisce alpha = Missing["NotFound"]*)
      ];
(*a_ significa prendi un elemento qualsiasi della lista e chiamalo a.
/; si legge "tale che e significa prendi un elemento a, ma solo se soddisfa quella condizione."*)
(* Se il numero di differenze raggiungibili \[EGrave] minore del numero
   totale di differenze, allora esiste almeno una beta impossibile. *)
    If[
      alpha === Missing["NotFound"],
      "Nessun differenziale impossibile trovato", (*se non \[EGrave] stato trovato nessun differenziale impossibile, allora la funzione restituisce "Nessun differenziale impossibile trovato"*)
      <|
        "Alpha" -> alpha,
        "BetaImpossibile" ->
          First[
            Complement[
              differenze,
              differenzeDopoRound[alpha, nBits, rounds](*prendi tutte le differenze possibili e togli quelle raggiungibili.*)
            ]
          ],
        "Rounds" -> rounds
      |>
  ...
  verificaDifferenzialeImpossibile[alpha_Integer, beta_Integer, nBits_Integer, rounds_Integer] :=
 Module[
  {differenze, raggiungibili},

  differenze = Range[1, 2^nBits - 1];
  raggiungibili = differenzeDopoRound[alpha, nBits, rounds];

  <|
   "Alpha" -> alpha,
   "Beta" -> beta,
   "Rounds" -> rounds,
   "NumeroBetaRaggiungibili" -> Length[raggiungibili],
   "NumeroBetaTotali" -> Length[differenze],
   "ImpossibileQ" -> Not[MemberQ[raggiungibili, beta]]
   |>
  ]
